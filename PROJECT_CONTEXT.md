...
> Son doğrulanan standart web/Poki kaynak paketi: `BUILD.json` sürüm **1.7.0** (24 Eylül 2026). Bu dosya yeni bir AI agent'a veya geliştiriciye verilecek teknik başlangıç noktasıdır. Geçmiş konuşmadaki tüm isteklerin aynen geçerli olduğunu varsaymayın: aşağıda mevcut kodun gerçekten yaptığı işler anlatılır. Davranış ile bu belge çelişirse kaynak kod ve `render_game_to_text()` çıktısı esas alınır.

## 1. Hangi proje, hangi sürüm?

- **Aktif standart web/Poki kaynağı:** `C:\Users\Pc\Desktop\ELEMENTAL RUN Browser v1.3 relaeses 2\ELEMENTAL RUN Poki v1.5.0\`. Klasörün adında v1.5.0 kalsa da içindeki `BUILD.json`, HTML cache anahtarları ve menü etiketi v1.7.0'dır. Yeni sürüm için sürekli yeni klasör açılmıyor.
...
5. Yükleme sonrasında tek `PLAY` düğmesi 3D lobiyi açar; doğrudan eğitimi başlatmaz. İlk eğitim sağdaki EĞİTİM portalından başlar; tamamlandıktan sonra aynı portal seçili LEVEL'i gösterir. Lobide mouse sürükleyerek serbest kamera kullanılır, pointer lock alınmaz. Koşucunun mouse kamera tercihi ayrı ve varsayılan kapalı kalır. Cache değişikliğinde `index.html` query `?v=...`, `service-worker.js`, `BUILD.json` birlikte güncellenmelidir.
...
- Durumlar: `gate → exploring → preparing → transition → inactive`. PLAY yalnız ilk girişte görünür; koşudan ana menüye dönüş doğrudan lobiye gider. `stepGame` lobi açıkken sadece lobi update/render ve ses miksini çalıştırır. Koşucu simülasyonu durur, SDK gameplayStop çağrılır.
- WASD/oklar kamera yönüne göre hareket eder; fare sürükleme orbit; ESC ayarlar. Mobilde sol joystick ve sağ alanda sürükleme, kamera merkezleme düğmesi vardır. Gamepad sol çubuk hareket, sağ çubuk orbit, Start ayarlar. Oda sınırları ve standlar için basit XZ AABB, sabit zemin Y kullanılır.
- Sekiz eldiven standı hazır şeffaf önizlemeler gösterir. Yakındaki gerçek GLB'ler tek tek, avatar durduğunda yüklenir; aynı anda en fazla iki GLB sahnede tutulur. Uzaktaki modellerin geometri/materyal/dokuları serbest bırakılır. Loader epoch kontrolü geç gelen eski isteğin sahneye dönmesini engeller. Koşucunun el önbelleği ayrı olduğundan yanlışlıkla dispose edilmez.
...
- Eski DOM ana navigasyon gizlidir; ortak mağaza/ayarlar DOM'u ekonomi ve erişilebilirlik için korunur. Yayın staging'inde `lobby-hub.css` de `assets/runtime/` altına gider; arka plan yolu `../../elemental run icons/...` yapılmalıdır.
